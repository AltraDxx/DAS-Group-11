...
```{r}
model_aroma_acidity_altitude <- glm(Qualityclass ~ aroma + acidity + altitude_mean_meters, data = data, family = binomial(link = "logit"))
plot_model(model_aroma_acidity_altitude, type = "pred",
           terms = c("aroma", "acidity", "altitude_mean_meters"))
...
quality_counts <- Data1 %>%  
  ...
  spread(Qualityclass, count, fill = 0) %>%
  mutate(proportion_good = Good / (Good + Poor))
ggplot(quality_counts, aes(x =harvested, y = proportion_good, fill = harvested)) +
  geom_bar(stat = "identity",show.legend = FALSE) +
...
```{r}
# Principal principal component analysis (PCA) for 'aroma', 'flavor', 'acidity'
data_pca <- data %>%
...
## Logistic Regression Model for Qualityclass Prediction
