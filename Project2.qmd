...
Preprocessing the data and conducting summary statistics.
...
q1_flavor <- quantile(Data$flavor, 0.25)
q3_flavor <- quantile(Data$flavor, 0.75)
...
q1_acidity <- quantile(Data$acidity, 0.25)
q3_acidity <- quantile(Data$acidity, 0.75)
...
q1_defects <- quantile(Data$category_two_defects, 0.25)
q3_defects <- quantile(Data$category_two_defects, 0.75)
...
q1_altitude <- quantile(Data$altitude_mean_meters, 0.25)
q3_altitude <- quantile(Data$altitude_mean_meters, 0.75)
...
ggplot(data_harvested, aes(x=Qualityclass, y = after_stat(prop), group=harvested, fill=harvested)) + 
    geom_bar(position="dodge", stat="count") +
    labs(y = "Proportion")
```
...
data_pca_final <- data.frame(pca_result_selected,country_category=data$country_category,category_two_defects=data$category_two_defects,altitude_mean_meters=data$altitude_mean_meters, Qualityclass = data_pca$Qualityclass)
...
```{r}
# optimal model
optimal_model <- glm(Qualityclass ~ PC1 + country_category, family=binomial(link = "logit"), data = data_pca_final)
optimal_model %>%
  summary()
```
