...
model_full <- glm(Qualityclass ~ country_of_origin + aroma + flavor + acidity + category_two_defects + altitude_mean_meters + harvested, data = Data1, 
...
```{r}
# Create a new variable 'country_category' based on 'country_of_origin' and convert 'country_category' to a factor and specify level order
data <- Data1 %>%
  mutate(country_category = ifelse(country_of_origin %in% c("Burundi",  "Colombia", "Thailand"), country_of_origin, "other")) %>%
  dplyr::select(country_of_origin, country_category, everything())
data$country_category <- factor(data$country_category, levels = c("Burundi", "Colombia", "Thailand", "other"))
```

## Interaction effects plots

```{r}
model_country_aroma_flavor <- glm(Qualityclass ~ country_category + aroma + flavor, data = data, family = binomial(link = "logit"))
plot_model(model_country_aroma_flavor, type = "pred",
           terms = c("aroma", "flavor", "country_category"))
```

```{r}
model_country_acidity_altitude <- glm(Qualityclass ~ country_category + acidity + altitude_mean_meters, data = data, family = binomial(link = "logit"))
...
p0 <- ggplot(data_country_category, aes(x = Qualityclass, y = after_stat(prop), group = country_category, fill = country_category)) + 
...
```{r}
# Fit logistic regression model with 'country_category' predictor and 'Qualityclass' response
...
data_pca_final <- data.frame(pca_result_selected, country_category = data$country_category, category_two_defects = data$category_two_defects, altitude_mean_meters = data$altitude_mean_meters, harvested = data$harvested, Qualityclass = data_pca$Qualityclass)
...
```{r}
# Conduct a data-processed model
model_full_after <- glm(Qualityclass ~ country_category + aroma + flavor + acidity + category_two_defects + altitude_mean_meters + harvested, data = data, family = binomial(link = "logit"))
...
```{r}
# Fit logistic regression model with PCA components
pca_model1 <- glm(Qualityclass ~ PC1 + PC2 + category_two_defects + altitude_mean_meters, data = data_pca_final, family = binomial(link = "logit"))

# Summarize the model
pca_model1 %>%
  summary()
pca_model_summary1 <- glance(pca_model1)
kable(pca_model_summary1, digits = 2)
...
optimal_model <- glm(Qualityclass ~ PC1 + country_category + category_two_defects + harvested, family = binomial(link = "logit"), data = data_pca_final)
optimal_model %>%
  summary()
...
-   $ountry\_category$, $category\_two\_defects$, and $harvested$ are the predictor variables
